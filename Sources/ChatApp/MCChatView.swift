...
struct MCChatView: View {
    ...
    /// Dùng để phân biệt "người dùng thật sự bấm back/rời màn hình chat" (lúc đó mới ngắt kết nối)
    /// với "app bị đưa xuống nền" — 2 trường hợp SwiftUI đôi khi gọi onDisappear giống hệt nhau,
    /// khiến app tự ngắt kết nối oan khi người dùng chỉ bấm Home / chuyển app khác.
    ...
    var body: some View {
        ...
        .onDisappear {
            // Chỉ ngắt kết nối khi người dùng thật sự rời màn hình chat (bấm back / đóng tab)
            // lúc app đang ở foreground. Nếu app đang bị đưa xuống nền (Home / chuyển app),
            // scenePhase lúc này KHÔNG phải .active — bỏ qua để giữ kết nối sống, kết hợp
            // với BackgroundKeepAlive (âm thanh im lặng) trong MCClient.
