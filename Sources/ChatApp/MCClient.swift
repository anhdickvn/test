...
final class MCClient: ObservableObject {
    ...
    // MARK: - Tự động gửi /login bằng mật khẩu đã lưu

    private func attemptAutoLogin() {
        guard let password = MCCredentialStore.loadPassword(host: host, port: port, username: username) else { return }
        let token = connectAttemptToken
        appendLog(.info, "Tìm thấy mật khẩu đã lưu cho tài khoản này, sẽ tự động /login sau khi vào server...")
    ...
    private func handleWindowItems(_ body: Data) {
        ...
        if windowId == 0 {
            for slot in 0..<Int(count) {
                guard let hbIndex = hotbarIndex(forInventorySlot: slot) else {
                    // vẫn phải đọc hết để giữ đúng vị trí con trỏ, dù không quan tâm slot này
                    guard let (_, next) = MCSlotParser.parse(body, from: idx, hotbarIndex: -1) else { return }
                    idx = next
                    continue
                }
    ...
    private func handleSetSlot(_ body: Data) {
        ...
        if windowIdSigned == 0 {
            guard let hbIndex = hotbarIndex(forInventorySlot: slot) else { return }
