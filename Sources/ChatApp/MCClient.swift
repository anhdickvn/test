...
final class MCClient: ObservableObject {
    ...
    private var shouldRestoreConnectionOnForeground = false
    ...
    private func setupBackgroundObservers() {
        ...
        backgroundObservers.append(nc.addObserver(forName: UIApplication.didEnterBackgroundNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor in
                ...
                self.shouldRestoreConnectionOnForeground = self.state == .connected
                guard self.shouldRestoreConnectionOnForeground else { return }
        ...
        backgroundObservers.append(nc.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor in
                ...
                // Nếu iOS/server làm TCP rớt trong lúc app ở nền, tự kết nối lại khi quay lại.
                if self.shouldRestoreConnectionOnForeground &&
                    (self.state == .disconnected || self.state.isFailed) {
                    self.shouldRestoreConnectionOnForeground = false
                    self.connect(host: self.host, port: self.port, username: self.username)
                } else {
                    self.shouldRestoreConnectionOnForeground = false
                }
            }
    ...
    func clickWindowSlot(_ slot: Int) {
        ...
        payload.append(0)                                        // mouseButton = 0 (chuột trái)
        ...
        let task = URLSession.shared.downloadTask(with: url) { [weak self] tempURL, _, error in
            Task { @MainActor in
                ...
                do {
                    ...
                    self.appendLog(.info, "Đã tải xong texture pack (\(fileName)). Lưu ý: app này chỉ chat nên chưa tự áp dụng texture khi hiển thị, file đã sẵn ở bộ nhớ app để dùng nơi khác nếu cần.")
    ...
    private func handleWindowItems(_ body: Data) {
        ...
        for slot in 0..<Int(count) {
            ...
            idx = next
            if let item {
                window.items[slot] = MCOpenWindowItem(slot: slot, itemId: item.itemId, nameSegments: item.nameSegments, rawSlotBytes: raw)
            } else {
...
// Bảng màu Minecraft được chỉnh độ sáng cho nền tối. Màu gốc quá tối (đặc biệt §0, §1,
// §2...) sẽ gần như mất chữ khi app dùng nền đen, nên giữ đúng "ý nghĩa" màu nhưng tăng
// contrast để chat vẫn đọc rõ trên iPhone.
private let mcColorHex: [Character: String] = [
    "0": "#6B7280", "1": "#60A5FA", "2": "#4ADE80", "3": "#22D3EE",
    "4": "#F87171", "5": "#C084FC", "6": "#F59E0B", "7": "#D1D5DB",
    "8": "#9CA3AF", "9": "#93C5FD", "a": "#86EFAC", "b": "#67E8F9",
    "c": "#FB7185", "d": "#E879F9", "e": "#FDE047", "f": "#F9FAFB",
]

private let mcNamedColorHex: [String: String] = [
    "black": "#6B7280", "dark_blue": "#60A5FA", "dark_green": "#4ADE80", "dark_aqua": "#22D3EE",
    "dark_red": "#F87171", "dark_purple": "#C084FC", "gold": "#F59E0B", "gray": "#D1D5DB",
    "dark_gray": "#9CA3AF", "blue": "#93C5FD", "green": "#86EFAC", "aqua": "#67E8F9",
    "red": "#FB7185", "light_purple": "#E879F9", "yellow": "#FDE047", "white": "#F9FAFB",
]
...
private extension MCConnectionState {
    var isFailed: Bool {
